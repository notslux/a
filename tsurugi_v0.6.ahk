...
Msgbox, cracked with easy by truly yours, clonazepam
Msgbox, dtbfed get out of my pc dtbfed! stop blindly using vmprotect thinking it can save your shitty pasted code!
Loaded := true
if (Loaded)
{
class JSON
...
Saturation := config.Tsurugi.Color.Saturation
...
Stabilizer = config.Tsurugi.Easing.Stabilizer.Sensitivity
FallbackPixelX := 0
FallbackPixelY := 0
...
{
EndpointX := Floor(coordinatemode(LookAtX + PredictionX, CameraToGunFOV, A_ScreenWidth, A_ScreenHeight) * (SmoothnessX/SmoothingDividerX) * SmoothingReplicatorX) - Stabilizer
EndpointY := Floor(coordinatemode(LookAtY + PredictionY, CameraToGunFOV, A_ScreenWidth, A_ScreenHeight) * (SmoothnessY/SmoothingDividerY) * SmoothingReplicatorY) - Stabilizer
}
